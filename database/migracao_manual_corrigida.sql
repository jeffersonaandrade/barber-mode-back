-- ===== MIGRAÇÃO MANUAL CORRIGIDA - EXECUTAR NO SQL EDITOR DO SUPABASE =====
...
-- ===== 5. MIGRAR DADOS EXISTENTES (COM VERIFICAÇÕES) =====
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Tabela clientes não existe, pulando migração de clientes';
    END IF;
END $$;

-- Verificar se a tabela avaliacoes existe antes de migrar
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'avaliacoes') THEN
        ...
        RAISE NOTICE 'Avaliações migradas com sucesso';
    ELSE
        RAISE NOTICE 'Tabela avaliacoes não existe, pulando migração de avaliações';
    END IF;
END $$;

-- Verificar se a tabela historico_atendimentos existe antes de migrar
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'historico_atendimentos') THEN
        ...
        RAISE NOTICE 'Histórico migrado com sucesso';
    ELSE
        RAISE NOTICE 'Tabela historico_atendimentos não existe, pulando migração de histórico';
    END IF;
END $$;

-- ===== 6. CRIAR DADOS PADRÃO =====
