...
async function filaRoutes(fastify, options) {
  ...
  /**
   * @swagger
   * /api/fila/finalizar-atendimento/{clienteId}:
   *   post:
   *     tags: [fila]
   *     summary: Finalizar atendimento de um cliente (PRIVADO)
   *     security:
   *       - Bearer: []
   *     parameters:
   *       - in: path
   ...
   *         description: Acesso negado
   */
  fastify.post('/fila/finalizar-atendimento/:clienteId', {
    preHandler: [fastify.authenticate]
  }, async (request, reply) => {
    try {
      const { clienteId } = request.params;
      ...
      const { data: cliente, error: clienteError } = await fastify.supabase
        ...
        .select('*')
        .eq('id', clienteId)
        .eq('barbeiro_id', userId) // Apenas o barbeiro que iniciou pode finalizar
        .single();
        
      if (clienteError || !cliente) {
        return reply.status(404).send({
          success: false,
          error: 'Cliente não encontrado ou você não iniciou o atendimento'
        });
      }
